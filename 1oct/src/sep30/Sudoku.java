...
public class Sudoku {
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < arr.length; i++){
			for(int j = 0; j < arr[i].length; j++){
				if(arr[i][j] != 0){
					makeAChoice(arr, i * arr.length + j + 1, 
							    rownos, colnos, sbnos, arr[i][j]);
				}
			}
		}
	...
	public static void sudoku(int[][] arr, int cellno, 
							  int[] rownos, int[] colnos, int[] sbnos) {
		if (cellno == 82) {
			for(int i = 0; i < arr.length; i++){
				for(int j = 0; j < arr[i].length; j++){
	...
	public static void makeAChoice(int[][] arr, int cellno, 
								   int[] rownos, int[] colnos, int[] sbnos, 
								   int choice) {
		int rno = (cellno - 1) / arr.length;
		int cno = (cellno - 1) % arr.length;
